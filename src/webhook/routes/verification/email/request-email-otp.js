...
module.exports = async (req, res) => {
    const connection = await mysql.createConnection({
      ...
      const findExisting = await connection
        .query(findExistingQuery, [email_Address])
        .catch((err) => console.log(err));
      ...
      if (findExisting[0].length > 0) {
        ...
        await connection
          .query(updateQuery, [hashedOTP, Date.now(), Date.now() + 3600000, email_Address])
          .catch((err) => console.log(err));
        // connection.end();
        // return res.status(400).send({
        //   ok: false,
        ...
        await connection
          .query(otpQuery, [email_Address, hashedOTP, Date.now(), Date.now() + 3600000])
          .catch((err) => console.log(err));
      }
