...
function isValidPortalNotificationPayload(payload) {
  ...
  if (!payload.notification || typeof payload.notification !== 'object') return false;
...
function buildPortalNotificationContainer(payload, { status = 'unread' } = {}) {
  ...
  const timestampLine = createdAtUnix ? `<t:${createdAtUnix}:R>` : toDisplay(notification.created_at);
  ...
  container.addActionRowComponents((row) => row.setComponents(...actionButtons));
...
function buildDmDisabledContainer(payload) {
  ...
  const steps = [
    ...
    `<@${discordUserId}>, we tried to DM you **"${toDisplay(notification.title)}"** ` +
      `but your server **Direct Message** setting is turned off, so we couldn't reach you privately.`,
    '',
    '### How to enable Direct Messages',
    '1. Right-click this server\'s icon (or open **Server Settings** dropdown).',
    '2. Click **Privacy Settings**.',
    '3. Turn **ON** "Direct Messages" / "Allow direct messages from server members".',
    '4. Once enabled, future Portal notifications will arrive in your DMs.',
  ];

  const container = new ContainerBuilder()
    .setAccentColor(COLOR_UNREAD)
    .addTextDisplayComponents((textDisplay) => textDisplay.setContent(steps.join('\n')));
...
function upsertNotificationRow(db, payload) {
  ...
  `).run({
    ...
    link_url: isValidHttpsUrl(notification.link_url) ? notification.link_url : null,
    created_at: notification.created_at ?? null,
  });
}

function setNotificationDelivery(db, notificationId, { dmChannelId, messageId, status }) {
  db.prepare(`
...
function createPortalNotificationHandler({
  ...
  return async (req, res) => {
    try {
      ...
      const container = buildPortalNotificationContainer(payload, { status: 'unread' });
      ...
      try {
        ...
        return res.status(200).json({ ok: true, message: 'Portal notification delivered via DM' });
      } catch (dmError) {
      ...
      return res.status(500).json({ ok: false, message: 'Failed to deliver portal notification' });
    }
  };
}
...
module.exports.createPortalNotificationHandler = createPortalNotificationHandler;
module.exports.isValidPortalNotificationPayload = isValidPortalNotificationPayload;
module.exports.buildPortalNotificationContainer = buildPortalNotificationContainer;
