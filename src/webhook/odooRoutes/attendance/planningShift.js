...
const publishedShift = async (req, res) => {
  ...
  buffers.get(key).push({ id, start_datetime, payload: req.body, received_at: Date.now() });
...
const deletedPlanningShift = async (req, res) => {
  try {
    ...
    return res.status(200).json({ ok: true, message: 'Planning shift deleted' });
  } catch (error) {
...
const updatePlanningShift = async (payload, planningMessage) => {
  ...
  if (messageEmbed.data.fields && messageEmbed.data.fields.length > 0) {
    for (let i = 0; i < newFields.length; i++) {
      ...
      const oldField = messageEmbed.data.fields.find((field) => field.name === newField.name);
  ...
  newPlanningEmbed.setFields(newFields).setColor(roleColors[x_role_color] || 'Blurple');
...
async function flushWindow(key) {
  ...
  const events = bucket.sort((a, b) => new Date(a.start_datetime) - new Date(b.start_datetime));
