    ...
    try {
      ...
      try {
        ...
        if (!selectResult.length) {
          throw new Error(
            "The scanned **`PRODUCT SKU`** does not match any product in the **`PENDING INVENTORY IN`** database."
          );
        }
        ...
        const insertQuery = `INSERT IGNORE INTO Completed_Inventory_In (ID, ORDER_ID, PRODUCT_SKU, PRODUCT_NAME, ORDER_CREATED, PLATFORM, PRODUCT_COGS, SCAN_DATE) VALUES (?, ?, ?, ?, ?, ?, ?, ?)`;
        await inv_connection.query(insertQuery, valuesToInsert);

        const deleteQuery = `DELETE FROM Pending_Inventory_In WHERE ID = ?`;
...
function buildSuccessEmbed(message, scannedProduct) {
  ...
  const embed = new EmbedBuilder()
    .setDescription(`## INVENTORY IN SCANNED`)
    .addFields([
      {
        name: "ORDER ID",
